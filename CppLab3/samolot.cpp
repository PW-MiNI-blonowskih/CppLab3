...
void Samolot::przygotuj_do_startu(int wysiada, int wsiada, const char* port) {
	...
	if (liczba_pasazerow < wysiada) {
		liczba_pasazerow = 0;
		std::cout << "ERR:Wysiadlo wiecej osob niz jest na pokladzie" << std::endl << "\tliczba pasazerow ustawiona na 0" << std::endl;
	} 
	else {
	...
	if (liczba_pasazerow + wsiada > MAX_LICZBA_PASAZEROW) {
		...
		std::cout << "ERR:Za duzo pasazerow, liczba ustawiona na MAX" << std::endl;
	}
	else {
...
// ================================================
// TODO: spytac o printowanie tego portu docelowego
// ================================================
std::ostream& operator<<(std::ostream& out, const Samolot& samolot) {
	std::ostream& returnStream = out << "Samolot nr " << samolot.id << " do " << /* samolot.port_docelowy << */" z " << samolot.liczba_pasazerow << " pasazerami na pokladzie ";

	switch (samolot.status)
	{
	case Samolot::status_samolotu::GOTOWY:
		return returnStream << "gotowy";
		break;
